...
export default function TimerItem(props) {
    ...
    React.useEffect(() => {
        if(reps === data.reps){
            if(data.sets === sets){
                ...
                setTimeout(() => {
                    setRest(true);
                    Speech.speak(`Set ${sets} completed. Rest for ${data.rest} seconds.`, options); // Speak when resting
                }, 1000);
            }
        }
    }, [reps]);
    ...
    React.useEffect(() => {
        if(timeLeft === 0){
            if(sets < data.sets){
                ...
                setTimeLeft(60);
                setRest(false);
                Speech.speak(`Begin Set ${sets + 1}`, options); // Announce the beginning of the next set
            }
        }
    }, [timeLeft]);
    ...
    const speakText = () => {
        if (Strings.ST132) {
            Speech.speak(Strings.ST132,options);
        }
    };
    useEffect(() => {
        if (finished && Strings.ST132) {
            Speech.speak(Strings.ST132,options);
        }
    }, [finished, Strings.ST132]);
    const renderRest = () => {
        if(rest && !finished){
            return (
    ...
    return (
        <View style={{flex: 1, alignContent:'center', alignItems:'center', width:width, height: height}}>
            ...
            {finished ? (
                <Animatable.View animation="slideInDown" style={{ maxWidth: 300, height: height, flex: 1, alignContent: 'center', alignItems: 'center', flexDirection: 'row' }}>
                    <View style={{ flexDirection: 'column', alignItems: 'center', marginTop: -70 }}>
                        ...
                        <Text style={{ fontSize: 34, fontWeight: 'bold', textAlign: 'center' }} onPress={speakText}>{Strings.ST132}</Text>
                    </View>
                </Animatable.View>
            ) : null}
